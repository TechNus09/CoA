...
for m in range(0,7):
    ...
    for k in range(0,n):  
        ...
        request = Request(url+skis[m]+'.json?p='+str(k), headers=headers)
        ...
        for i in range(0,20): 
            ...
            if fdata[i]["name"] == name :
                ...
                results[m]['xp'] =xp=fdata[i]["xp"]
                #get lvl % from xp
                results[m]['lvl'] = tabfill(xp)[0]
                results[m]['lvl%'] = tabfill(xp)[1]
                print('ur lvl is ',tabfill(xp)[0],'-',tabfill(xp)[1],'%')
                print("----------------------------------------------")
                test=True
                break
            else:
